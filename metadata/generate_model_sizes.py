...
def render_manual_sizes(rows: list[ParameterRow]) -> str:
    """Render the literal total-parameter map from all canonical CSV rows."""
    lines = [
        BEGIN_MARKER,
    ...
    lines.extend(
        f"    {row.model_id!r}: {row.total_literal}," for row in rows
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if current != expected:
            ...
            return 1
        print(
            f"Validated {len(rows)} parameter rows and {size_count} "
    ...
    print(f"Updated {args.module} with {size_count} MANUAL_SIZES entries.")
    return 0
